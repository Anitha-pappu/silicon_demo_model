...
def get_shap_explanation(
    model: CustomModelWrapper, data: pd.DataFrame,
) -> shap.Explanation:
    ...
    shap_values_for_original_columns = _extract_shap_values_for_original_columns(
        explanation=explanation, original_columns=data.columns,
    )
...
def _extract_shap_values_for_original_columns(
    explanation: shap.Explanation, original_columns: tp.List[str],
) -> pd.DataFrame:
