...
class _XAxisGrid(object):
    ...
    def get_axis_location(self, plot_id: int) -> tp.Dict[str, tp.Union[str, float]]:
        ...
        if plot_id % 2 == 0:
            return dict(side="left", position=self.left_domain - offset)
        return dict(side="right", position=self.right_domain + offset)
...
class _YAxisGrid(object):
    ...
    @staticmethod
    def _get_y_domains(
        n_subplots: int, vertical_gap_between_subplots: float,
...
def _get_single_layout(
    ...
    return {
        ...
        **{
        ...
        axes_references.y_axes[0]: dict(
            anchor=axes_references.anchor_x, domain=list(height_ratio),
        ),
    }
