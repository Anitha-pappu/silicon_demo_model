...
def _draw_confidence_interval(
    ...
    fig.add_trace(
        go.Scatter(name=upper_bound_name, x=data[x], y=data[y_max])
        .update(bound_params),
    )
    fig.add_trace(
        go.Scatter(
    ...
    for _, unit_data in data.groupby(units):
        fig.add_trace(
            go.Scatter(
                ...
                name=legendname,
                mode="lines",
                line=dict(color=cast_color_to_rgba(color, opacity)),
...
def plot_lines(  # noqa: WPS211,WPS231
    ...
    if is_range_plot:
        fig.update_layout(
            title=title if title else f"Range Plot for Sensor {y}", hovermode="x",
        )
