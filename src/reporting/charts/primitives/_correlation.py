...
def _get_correlation_data(
    data: pd.DataFrame, method: str, mask: tp.Optional[MaskMode],
) -> pd.DataFrame:
    ...
    corr_data = data.corr(method=method, numeric_only=True)
...
def plot_correlation(  # noqa: WPS211
    ...
    annotate_correlations: bool = False,
    ...
    Args:
        ...
        annotate_correlations: shows correlation values on the chart if set `true`
...
def _filter_input(
    data: pd.DataFrame, columns: tp.List[str], rows: tp.List[str],
) -> pd.DataFrame:
