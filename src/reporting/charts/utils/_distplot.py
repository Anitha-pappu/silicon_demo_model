...
def calculate_optimal_bin_width(
    ...
    Args:
        data: data to calculate opt width for
        search_bounds: the min and max size of the bin,
            if not provided (None), then they are estimated
        search_precision: is the number of candidate bin widths to be generated and
            examined in order to find the ideal number

    Notes:
        * If data contains nans, such values will be ignored.
        ...
        * Once the search bounds are defined, a number ``search_precision`` of candidate
          bin widths are generated, by taking a uniform sampling between min and max of
          the search bounds.
    """
...
def _determine_bin_width_search_bounds(data: TVector):
    ...
    Notes:
        This function assumes the input ``data`` are of a numeric type.

        Handle the case of constant data by returning a bin width of 1
    """
...
def _convert_time_inputs_to_float(
    ...
    The float values are to be interpreted as seconds
    """
