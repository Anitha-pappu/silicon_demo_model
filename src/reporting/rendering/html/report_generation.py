...
TMetaData = tp.Union[None, tp.Dict[str, tp.Any], "ReportMetaData"]
...
def _load_jinja_template(
    template_path: tp.Optional[str], assets_path: tp.Optional[str],
) -> Template:
    """
...
class ReportMetaData(object):
    ...
    @staticmethod
    def from_input(metadata: TMetaData) -> ReportMetaData:
        if metadata is None:
    ...
    def _parse_time(
        self, time_of_creation: tp.Optional[tp.Union[str, datetime]],
    ) -> datetime:
        if time_of_creation is None:
            ...
            try:
                time_of_creation = datetime.strptime(
                    time_of_creation, self.DATE_TIME_FORMAT,
                )
            except ValueError:
                logger.warning(
