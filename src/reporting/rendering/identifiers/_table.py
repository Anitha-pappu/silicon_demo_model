...
_TColumnName = str | int
_TSortingConfig = list[tuple[_TColumnName, str]] | None
_TByColumnPrecision = dict[tp.Hashable, int]
_TPrecision = int | _TByColumnPrecision | None
...
class Table(object):
    ...
    Examples::
        ...
        Sort by `column_one` ascending and by `column_two` descending
        >>> df = pd.DataFrame({"col": [6, 4, 5]}, idnex=pd.Index([1, 3, 2], name="idx"))
        >>> Table(df, sort_by=[("col", "asc"), ("idx", "desc")])
    ...
    columns_filters_position: str | None = "footer"
    columns_to_color_as_bars: list[str] | None = None
    width: float = 50
