...
class ParticleSwarmSolver(ScaledContinuousSolver):
    ...
    def __init__(
        ...
        if (
            ...
            and popsize != tp.cast(np.ndarray[tp.Any, tp.Any], init).shape[0]
        ):
        ...
        if isinstance(init, string_types):
            if init == "latinhypercube":
                self._particles = latin_hypercube(
                    (tp.cast(int, popsize), len(domain)),
                    self.rng,
                )
            elif init == "random":
                self._particles = uniform_random(
                    (tp.cast(int, popsize), len(domain)),
                    self.rng,
                )
            else:
        ...
        self.global_best_idx: tp.Optional[Vector] = None
        self.best_objective_values: tp.Optional[Vector] = None
    ...
    def update_best_positions(self) -> None:
        ...
        if self.best_objective_values is None:
            ...
            self.best_objective_values[better] = self._objective_values[better]  # type: ignore # noqa # pylint: disable=line-too-long
            self._best_positions[better] = self._particles[better]

        self.global_best_idx = np.argmin(self._objective_values)  # type: ignore
    ...
    def best(self) -> tp.Tuple[Vector, float]:
        ...
        return (
            ...
            tp.cast(float, self.objective_values[self.global_best_idx]),
        )
