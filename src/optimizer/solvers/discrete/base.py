...
from optimizer.types import MINIMIZE, TSense, Vector
...
class DomainMapper:
    ...
    def __init__(
        self,
        domain_map: tp.Dict[tp.Hashable, int],
        strict: bool = False,
        default: tp.Optional[int] = None
    ):
        """Constructor.
        ...
        for i in range(x.shape[0]):
            for j in range(x.shape[1]):
                if self.strict:
                    ...
                    mapped = self.domain_map.get(x[i, j], self.default)  # type: ignore
...
class IndexedDiscreteSolver(DiscreteSolver, ABC):
    # pylint: disable=abstract-method
    """
    ...
    @staticmethod
    def make_domain_matrix(domain: tp.List[tp.List[tp.Hashable]]) -> Vector:
    ...
    def indices_to_domain(
        index_matrix: Vector, domain_matrix: Vector
    ) -> Vector:
    ...
    @staticmethod
    def make_domain_map(
        domain: tp.List[tp.List[tp.Hashable]],
        ...
        for dimension in domain:
            for index, domain_value in enumerate(dimension):
                if isinstance(domain_value, tp.Hashable):
    ...
    @staticmethod
    def wrap_domain_map(
        domain_map: tp.Dict[tp.Hashable, int], strict: bool = True, default: int = 0,
    ) -> tp.Callable[[Vector], Vector]:
