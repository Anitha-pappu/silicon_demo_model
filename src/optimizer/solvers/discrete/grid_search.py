...
class GridIterator(abc.ABC):
    ...
    def __init__(
        ...
        self.grid: tp.Optional[Vector] = None
...
class LowMemoryGridIterator(GridIterator):
    ...
    def __next__(self) -> Vector:
        ...
        while n < batch and n + self.points_returned < grid_size:
            out.append(next(self.grid))  # type: ignore
            n += 1
...
class GridSearchSolver(IndexedDiscreteSolver):
    ...
    def __init__(
        ...
        self.current_grid_points: tp.Optional[Matrix] = None
        self.current_objective_values: tp.Optional[Vector] = None

        self.best_point: tp.Optional[Vector] = None
        self.best_objective = float("inf")

    @staticmethod
    def n_grid_points(domain: tp.Sequence[tp.Sized]) -> int:
    ...
    def grid_fits_in_numpy_array(grid_size: int) -> bool:
        ...
        """
        return grid_size * np.intp().nbytes < np.iinfo(np.intp).max  # pylint: disable=E1120
