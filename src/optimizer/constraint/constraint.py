...
from optimizer.types import Matrix, Predictor, ReducesMatrixToSeries, Vector
from optimizer.utils.functional import column
from optimizer.utils.validation import check_matrix

TConstrainFunc = tp.Union[ReducesMatrixToSeries, Predictor]
TConstrainSet = tp.Union[ConstraintSet, tp.List[tp.Any], Vector]
TConstraintCompatible = tp.Union[float, ReducesMatrixToSeries, Predictor, str]
TConstrainDefinition = tp.Union[
    tp.Tuple["BaseConstraint"],
    tp.Tuple[TConstraintCompatible, str, TConstraintCompatible],
    tp.Tuple[
        TConstraintCompatible,
        str,
        TConstraintCompatible,
        str,
        TConstraintCompatible,
...
class BaseConstraint(abc.ABC):
    ...
    def __init__(
        self,
        constraint_func: TConstrainFunc,
        ...
        if not callable(constraint_func):
            if (
                not hasattr(constraint_func, "predict")
                or not callable(constraint_func.predict)
            ):
        ...
        self.constraint_values: Vector
        self.distances: tp.Optional[Vector] = None

    @property
    def constraint_func(self) -> ReducesMatrixToSeries:
        """ Resolved constraint function """
        return tp.cast(ReducesMatrixToSeries, self._resolve_attr(self._constraint_func))
    ...
    @staticmethod
    def _resolve_attr(obj: tp.Any) -> tp.Any:
        """
...
class InequalityConstraint(BaseConstraint):
    """
    Represents a non-strict inequality constraint.
    """

    def __init__(
        self,
        constraint_func: TConstrainFunc,
        lower_bound: tp.Optional[TConstraintCompatible] = None,
        upper_bound: tp.Optional[TConstraintCompatible] = None,
        name: tp.Optional[str] = None,
    ) -> None:
        """Constructor.
        ...
        Args:
            ...
            lower_bound: lower bound of the constraint.
                         - float: constant value for the lower bound.
            ...
            upper_bound: upper bound of the constraint.
                         - float: constant value for the lower upper.
                         - Callable: will be called before comparing to inputs.
                         - Predictor: same as callable but using predict method.
            name: optional name for logging.
...
class EqualityConstraint(BaseConstraint):
    ...
    def __init__(
        self,
        constraint_func: TConstrainFunc,
        equality: tp.Optional[tp.Union[TConstrainFunc, float]] = None,
        ...
        Args:
            ...
            equality: equality constraint specifier.
                      - float: constant value for the equality.
                      - Callable: will be called before comparing to inputs.
                      - Predictor: same as callable but using predict method.
...
def one_sided_constraint(
    lhs_constrain: TConstraintCompatible,
    comp: str,
    rhs_constrain: TConstraintCompatible,
    equality_epsilon: float = 1e-6,
    name: tp.Optional[str] = None,
) -> BaseConstraint:
    ...
    Args:
        lhs_constrain: left hand side of constraint.
             - float: constant value for the lower bound.
             - Callable: will be called before comparing to inputs.
             - Predictor: same as callable but using predict method.
             - str: used to index a passed DataFrame.
        comp: str, comparator or "in" for set inclusion logic.
        rhs_constrain: right hand side of constraint.
             - float: constant value for the lower bound.
             - Callable: will be called before comparing to inputs.
             - Predictor: same as callable but using predict method.
    ...
    """
    lhs = column(lhs_constrain) if isinstance(lhs_constrain, str) else lhs_constrain
    rhs = column(rhs_constrain) if isinstance(rhs_constrain, str) else rhs_constrain
    ...
    if not any(
        [
            callable(lhs),
            callable(rhs),
            isinstance(lhs, Predictor),
            isinstance(rhs, Predictor),
        ]
    ):
        raise InvalidConstraintError(
    ...
    # As a result, it must be callable. In the case that it's a constant,
    # we can just wrap it as a function that always returns itself as an array.
    if not callable(lhs) and not isinstance(lhs, Predictor):
        lhs = tp.cast(ReducesMatrixToSeries, ConstantCallable(lhs))
    ...
    if comp in _LESS_THAN + _GREATER_THAN:
        ...
        constraint_: BaseConstraint = InequalityConstraint(
...
def two_sided_constraint(
    lhs: TConstraintCompatible,
    left_comp: str,
    middle: TConstraintCompatible,
    right_comp: str,
    rhs: TConstraintCompatible,
    name: tp.Optional[str] = None,
) -> BaseConstraint:
    """Constraint factory function.
    ...
    # we can just wrap it as a function that always returns itself as an array.
    if not callable(middle) and not isinstance(middle, Predictor):
        middle = tp.cast(ReducesMatrixToSeries, ConstantCallable(middle))
...
def constraint(
    *constraint_definition: TConstrainDefinition,
    ...
    Raises:
        InvalidConstraintError: if ``constraint_definition``'s length equals one but
            provided constraint isn't an instance of ``BaseConstraint``
        InvalidConstraintError: if ``constraint_definition`` length isn't equal one,
            three or five; those are the only valid definitions possible

    Returns:
        Constraint object with the expected properties.
    """
    if len(constraint_definition) == 1:
        if not isinstance(constraint_definition[0], BaseConstraint):
            raise InvalidConstraintError(
                "Length 1 constraint definitions must be an instance of BaseConstraint."
            )
        return constraint_definition[0]  # type: ignore

    elif len(constraint_definition) == 3:
        return one_sided_constraint(
            *constraint_definition,  # type: ignore
            equality_epsilon,
            name,
        )
    elif len(constraint_definition) == 5:
        return two_sided_constraint(*constraint_definition, name=name)  # type: ignore
    else:
        raise InvalidConstraintError(
