...
from optimizer.types import AllowedRealTypes, TReal, Vector
...
class MultiplesOf(ConstraintSet):
    ...
    def __init__(self, multiple: TReal):
        """Constructor.
...
class Integers(MultiplesOf):
    ...
    def __init__(self) -> None:
        """Constructor.
        """
        super(Integers, self).__init__(1.0)
...
class UserDefinedSet(ConstraintSet):
    ...
    def __init__(self, collection: tp.Union[tp.Iterable[TReal], Vector]) -> None:
        """Constructor.
        ...
        """
        invalid = [item for item in collection
                   if not isinstance(item, AllowedRealTypes)]
        if invalid:
            raise ValueError(
                f"Invalid arguments provided in user defined set: {invalid}"
            )

        self.collection = np.array(sorted(set(collection)))
...
class StepSizeSet(MultiplesOf):
    ...
    def __init__(self, step_size: TReal, center: TReal = 0.0) -> None:
        """Constructor.
