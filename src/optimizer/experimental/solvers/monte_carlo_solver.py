...
class MonteCarloSolver(Solver):
    ...
    def __init__(
        self,
        control_features: tp.Tuple[str, ...],
        sphere_df: pd.DataFrame,
        n_samples: int,
        domain: tp.List[TBounds],
        ...
        self._final_solutions: Vector = np.array([])
        self._final_objectives: Vector = np.array([])
    ...
    def _drop_constant_column(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def ask(self) -> Vector:
        ...
        return tp.cast(Vector, mc_sample_bounds_filtered.values)
