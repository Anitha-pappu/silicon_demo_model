...
from optimizer.types import MINIMIZE, Matrix, TBounds, TSense, Vector
...
class AdaptiveParameter:
    ...
    def give_feedback(self, previous_objective: Vector, new_objective: Vector) -> None:
        """
    ...
    def _process_feedback(
        self,
        previous_objective: Vector,
        new_objective: Vector,
    ) -> None:
...
class NormalNonAdaptiveParameter(AdaptiveParameter):
    ...
    def __init__(self, mu: float, sigma: float, seed: tp.Optional[int] = None) -> None:
        super().__init__()
        self._mu = mu
        self._sigma = sigma
...
class Mutator:
    def __init__(
        self,
        f: tp.Union[float, AdaptiveParameter],
        seed: tp.Optional[int] = None,
    ) -> None:
        """Constructor.
    ...
    def __call__(self, solutions: Vector, fitness: Vector) -> Vector:
        raise NotImplementedError("Need to override Mutator.__call__")

    def _mutation_parent_indexes(self, n: int, k: int) -> Vector:
...
class Crossover:
    ...
    def __init__(
        self,
        cr: tp.Union[float, AdaptiveParameter],
        seed: tp.Optional[int] = None,
    ) -> None:
        """Constructor.
    ...
    def __call__(self, solutions: Vector, mutants: Vector) -> Vector:
    ...
    def _combine(self, solutions: Vector, mutants: Vector) -> Vector:
...
class BinomialCrossover(Crossover):
    ...
    def _generate_forced_crossover_positions(self, n: int, k: int) -> Vector:
...
class AdaptiveStrategy:
    def __init__(self, strategies: tp.Dict[str, tp.Union[Crossover, Mutator]]) -> None:
        self._strategies = strategies

    def get_strategy(self) -> tp.Tuple[str, tp.Union[Crossover, Mutator]]:
...
class RandomAdaptiveStrategy(AdaptiveStrategy):
    def __init__(
        self,
        strategies: tp.Dict[str, tp.Union[Crossover, Mutator]],
        seed: tp.Optional[int] = None,
    ) -> None:
...
class AdaptiveBinomialCrossOver(AdaptiveStrategy):
    ...
    def _process_feedback(
        self, strategy: str, previous_objective: Vector, new_objective: Vector
    ) -> None:
        self._strategies[strategy].give_feedback(previous_objective, new_objective)
...
class AdaptiveDifferentialEvolutionSolver(ScaledContinuousSolver):
    def __init__(
        self,
        domain: tp.List[tp.Union[TBounds, RealDimension]],
        sense: TSense = MINIMIZE,
        seed: tp.Optional[int] = None,
        popsize: int = 10,
        maxiter: int = 100,
        init: tp.Union[str, Matrix] = "latinhypercube",
        mutator_strategy_picker: tp.Optional[AdaptiveStrategy] = None,
        crossover_strategy_picker: tp.Optional[AdaptiveStrategy] = None,
    ):
        ...
        self._current_mutation_strategy: tp.Optional[str] = None
        self._current_crossover_strategy: tp.Optional[str] = None
    ...
    def _default_mutation_strategies(self) -> tp.Dict[str, Mutator]:
        """
    ...
    def _default_crossover_strategies(self) -> tp.Dict[str, Crossover]:
        """
    ...
    def _init_population(self, init: tp.Union[str, Matrix]) -> Vector:
    ...
    def _mutate(self) -> Vector:
        ...
        Returns: Vector of same shape as self._solutions. These are the individuals
    ...
    def _crossover(self, mutants: Vector) -> Vector:
        """
        Given the mutants for each solution, perform the crossover by picking the
            adaptive crossover logic that corresponds.

        Args:
            mutants: Vector of shape self._solutions.shape

        Returns: Vector containing all trials, these are the solutions that the user
    ...
    def _give_feedback_to_mutation_picker(self, objective_values: Vector) -> None:
    ...
    def _give_feedback_to_crossover_picker(self, objective_values: Vector) -> None:
    ...
    def _update_current_trial(
        self, parameters: Vector, objective_values: Vector
    ) -> None:
        """
