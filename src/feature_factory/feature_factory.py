...
class DAGHandlerMixin(object):
    def __init__(self, config: tp.Dict[str, tp.Any], pydantic_model: BaseModel):
        """Mixin that handles parsing `config` according to `pydantic_model`,
        initializing `self._graph`, and providing a method to create a derived feature
        corresponding to a given node.

        Args:
            config (Dict[str, Any]): especification of eng features, constraints
             and models
            pydantic_model (BaseModel): pydantic model used to parse `config`
        """
        self._graph = self.build_graph(pydantic_model(cookbook=config))  # type: ignore
    ...
    def _evaluate_node(self, data: pd.DataFrame, node_name: str) -> pd.DataFrame:
        ...
        if predictor is not None:
            ...
            ans = recipe.function(
                data, recipe.dependencies, *recipe.args, **recipe.kwargs,
            )
        else:
...
class FeatureFactory(Transformer, DAGHandlerMixin):
    def __init__(  # noqa: WPS612
        ...
        pydantic_model: BaseModel = DerivedFeaturesCookBook,  # type: ignore
    ):
        """
        Creates the eng features specified by `config`. `config` must
        have the schema of `DerivedFeaturesCookBook`.

        Args:
            config (Dict[str, Any]): specification of eng features.
    ...
    def fit(
        self,
        data: pd.DataFrame,
        target: tp.Union[npt.NDArray["np.generic"], pd.Series, None] = None,
        **fit_params: tp.Any,
    ) -> Transformer:
        """
        ...
        Raises:
            ...
            FunctionReturnError: if the function associated
             with an eng feature doesn't create it
        ...
        self.topological_sort_: tp.List[str] = filtered_toposort  # noqa: WPS120
        return self
    ...
    def _not_leaf(self, node_name: str) -> bool:
        out_degree: int = self._graph.out_degree(node_name)
        return out_degree > 0
...
def create_features(params: tp.Dict[str, tp.Any], data: pd.DataFrame) -> pd.DataFrame:
...
def draw_graph(transformer: FeatureFactory, feature: tp.Optional[str] = None) -> None:
    ...
    graph = transformer.build_graph(
        transformer.pydantic_model(cookbook=feature_map),   # type: ignore
    )
