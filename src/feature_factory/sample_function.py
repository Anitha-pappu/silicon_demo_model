...
def pandas_divide(
    ...
    **kwargs: Any,
    ...
    Args:
        data (pd.DataFrame): data
        dependencies (Sequence[str]): list of columns for input
...
def pandas_qcut(
    ...
    **kwargs: Any,
    ...
    """
    Wrapper function to run pandas.qcut() on one column of a pandas dataframe

    Args:
        data: input data
        dependencies: name of column in a list input
        **kwargs: kwargs for pandas.qcut()
    ...
    if dependencies_count != 1:
        raise ValueError(
            "`dependencies` should contain only one variable"
            f" names: given {dependencies_count}.",
        )
    return pd.qcut(data[dependencies].squeeze(), **kwargs)
...
def pandas_cut(
    ...
    **kwargs: Any,
    ...
    """
    Wrapper function to run pandas.cut() on one column of a pandas dataframe

    Args:
        data: input data
        dependencies: name of column in a list input
        **kwargs: kwargs for pandas.cut()
    ...
    return pd.cut(data[dependencies].squeeze(), **kwargs)
