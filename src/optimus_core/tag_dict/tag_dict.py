...
TTagRow = tp.Union[tp.Dict[str, tp.List[str]], pd.DataFrame]
...
class TagDict(object):  # noqa: WPS214
    ...
    def __getitem__(self, tag: str) -> tp.Dict[str, tp.Any]:
        ...
        return tp.cast(tp.Dict[str, tp.Any], tag_data.to_dict())
    ...
    def select(  # noqa: WPS231
        ...
        def _condition(  # noqa: WPS430
            data: pd.DataFrame,
        ) -> tp.Union[pd.DataFrame, bool]:
    ...
    def select_group_by(
        ...
        group_by_select = data.groupby(group_by)[select].apply(list).to_dict()
        return tp.cast(tp.Dict[str, tp.List[str]], group_by_select)

    def get_tag_ranges(
        self, tags: tp.Optional[tp.Iterable[str]] = None,
    ) -> tp.Dict[str, TRange]:
        if tags is None:
    ...
    def _repr_html_(self) -> str:  # noqa: WPS120
        ...
        return (
            f"<b>{class_name}"
            f" data</b>:<tr>{html_representation}"
        )
