...
class CustomSequentialFeatureSelector(Transformer, SequentialFeatureSelector):
    """
    ...
    def __init__(
        ...
        *,
        n_features_to_select: tp.Optional[int] = None,
        direction: str = "forward",
        scoring: tp.Optional[tp.Any] = None,
        cv: int = 5,
        n_jobs: tp.Optional[int] = None,
        iter_till_improvement: bool = False,
        features_to_keep: tp.Optional[tp.List[str]] = None,
    ) -> None:
        """
        Args:
    ...
    def fit(  # noqa: WPS238,WPS210,WPS231
        self,
        x: pd.DataFrame,  # noqa: WPS111
        y: tp.Union[npt.NDArray[np.generic], pd.Series] = None,  # noqa: WPS111
        **fit_params: tp.Any,
    ) -> "CustomSequentialFeatureSelector":  # pylint:disable=unused-argument
        """
        ...
        if self.n_features_to_select is None:
            ...
            if not 0 < float(self.n_features_to_select) < n_features:
            ...
            if not 0 < float(self.n_features_to_select) <= 1:
                raise ValueError(error_msg)
    ...
    def _get_best_new_feature(
        self,
        estimator: Transformer,
        X: pd.DataFrame,  # noqa: WPS111,N803
        y: pd.DataFrame,  # noqa: WPS111
        current_mask: npt.NDArray[np.generic],
        features_to_keep_idx: tp.List[str],
    ) -> tp.Tuple[str, float]:
