...
class SimpleNegControlSelector(Transformer):
    ...
    def __init__(
        self,
        estimator: tp.Optional[RegressorMixin] = None,
    ...
    def fit(
        ...
        **fit_params: tp.Any,
        ...
        Fits the selector by running an estimator on the given dataframe and additional
            random features, and selecting those features in the data that have higher
        ...
        if y is None:
            ...
            raise ValueError(f"{class_name} requires target")

        x_rnd = self.append_random_features(x)
        self.estimator.fit(x_rnd.values, y.values, **fit_params)
    ...
    def get_support(self) -> npt.NDArray[np.generic]:
...
class BootstrapNegControlSelector(Transformer):
    ...
    @threshold.setter
    def threshold(self, value_to_set: float) -> None:
    ...
    def fit(
        ...
        **fit_params: tp.Any,
        ...
        feat_imp_perc = feat_imp_counts / np.array(self.n_bootstrap_samples)
    ...
    def _run_bootstrap(  # noqa: WPS210
        ...
        **fit_params: tp.Any,
        ...
        for _ in range(self.n_bootstrap_samples):
            ...
            model.fit(x_bootstrap.values, y_bootstrap.values, **fit_params)
...
class NonParametricNegControlSelector(Transformer):
    ...
    def __init__(
        self,
        estimator: tp.Optional[Estimator] = None,
    ...
    @significance.setter
    def significance(self, value_to_set: float) -> None:
    ...
    def fit(
        ...
        **fit_params: tp.Any,
        ...
        bootstrap_importances = self._run_bootstrap(x, y, x_cols, **fit_params)
        feat_significances = self._sign_test(bootstrap_importances, x_cols)

        mask = np.ones_like(feat_significances, dtype=bool)
        mask[feat_significances > np.array(self.significance, dtype=np.float32)] = False
    ...
    def _run_bootstrap(
        ...
        **fit_params: tp.Any,
        ...
        bootstrap_importances = pd.DataFrame(
            {},
            columns=x_cols + [self.random_feat_name],
...
def _get_feature_importances(estimator: tp.Any) -> tp.Any:
