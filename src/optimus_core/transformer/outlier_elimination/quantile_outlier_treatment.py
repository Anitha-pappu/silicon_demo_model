...
class QuantileOutlierRemoval(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self,
        how: tp.Optional[str],
        low_lim: tp.Optional[float] = None,
        up_lim: tp.Optional[float] = None,
        td: tp.Optional[TDict] = None,
    ) -> None:
        """
        Args:
        ...
        self.up_lim = up_lim
        self.td = td
        self.lower_bounds: tp.List[float] = []
        self.upper_bounds: tp.List[float] = []

    def update_limits(self, data: pd.Series) -> None:  # noqa: WPS231
        """
        ...
        if is_numeric_dtype(data):
            ...
            self.lower_bounds.append(np.nan)
            self.upper_bounds.append(np.nan)
    ...
    def transform(
        ...
        for column_index in range(x.shape[1]):
            ...
            if is_numeric_dtype(x[x.columns[column_index]]):
                col[
                    (col < self.lower_bounds[column_index])
