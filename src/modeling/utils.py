...
def check_model_features(
    ...
    return model_features  # type: ignore
...
def create_mock_class_to_raise_import_error(
    missing_libs: str | list[str],
) -> object:
    """Create a mock class to raise an ImportError when instantiated.

    Args:
        missing_libs: list of missing libraries.

    Returns:
        Mock class.
    """

    class MockClass(object):  # noqa: WPS431
        def __init__(self, *args, **kwargs):  # type: ignore
            raise ImportError(
                f"The following libraries are missing: {missing_libs}."
                f" Please install them to use this feature.",
            )
