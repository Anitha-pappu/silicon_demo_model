...
class ByFracSplitter(SplitterBase):
    ...
    def __init__(
        ...
        shuffle: bool = False,
        stratify: tp.Optional[npt.NDArray["np.generic"]] = None,
    ...
    def _split(self, data: pd.DataFrame) -> tp.Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        test_split: tp.Tuple[pd.DataFrame, pd.DataFrame] = train_test_split(
            data,
            **self._train_test_split_kwargs,
        )
        return test_split  # noqa: WPS331

    def _sort_by_date(self, data: pd.DataFrame) -> pd.DataFrame:
        if self._datetime_column is None:
            raise ValueError(
                "Must provide datetime_column if sort_date"
                " is `True`. Got `None`.",
            )
        verify_column_inside_dataframe(data, self._datetime_column)
        data[self._datetime_column] = convert_datetime(
            data[self._datetime_column], **(self._to_datetime_kwargs or {}),
        )
