...
from modeling.api import ContainsEstimator, Estimator, SupportsModel
from modeling.types import Matrix
...
class Transforms(tp.Protocol):
    def transform(self, data: pd.DataFrame, **kwargs: tp.Any) -> Matrix:
        """Transforms data"""


@tp.runtime_checkable
class ModelThatContainsEstimatorAndTransforms(
    SupportsModel,
    ContainsEstimator,
    Transforms,
    tp.Protocol,
    ...
    """
    This is a protocol that describes
    a model type that contains estimator and is able to do data transformation
    """


class ModelThatContainsEstimator(
    SupportsModel,
    ContainsEstimator,
    tp.Protocol,
):
    """
    This is a protocol that describes
    a model type that contains estimator
    """


def plot_partial_dependency_for_sklearn(
    data: pd.DataFrame,
    model: ModelThatContainsEstimatorAndTransforms | ModelThatContainsEstimator,
    ...
    """
    if isinstance(model, ModelThatContainsEstimatorAndTransforms):
...
def _apply_transformation(
    data: pd.DataFrame,
    model: ModelThatContainsEstimatorAndTransforms,
    ...
    if drop_missing_values:
        transformed_data = _drop_rows_containing_nan(
            transformed_data, model.features_out,
        )
...
def _drop_rows_containing_nan(
    data: pd.DataFrame, features: tp.List[str],
) -> pd.DataFrame:
...
def _turn_off_spine_ticks(ax: plt.Axes) -> None:
...
def _identify_major_formatter(
    ax: plt.Axes,
    axis_index: int,
    n_columns: int,
    current_major_formatter: plt.Formatter | None,
) -> plt.Formatter:
    ...
    """
    is_first_axis_in_row = axis_index % n_columns == 0  # first (and main) axis in row
    if is_first_axis_in_row or current_major_formatter is None:
...
def _apply_style_to_axis(
    ax: plt.Axes,
    major_formatter: plt.Formatter,
    show_grid: bool,
    average_line_color: str,
    individual_lines_color: str,
) -> None:
    ...
    if legend:
        ...
        legend.legendHandles[0].set_color(average_line_color)  # type: ignore
...
def _apply_styles(
    ...
    for axis_index, ax in enumerate(non_empty_axes):
        major_formatter = _identify_major_formatter(
            ax, axis_index, n_columns, current_major_formatter=major_formatter,
        )
...
def _validate_data(model: SupportsModel, transformed_data: pd.DataFrame) -> None:
