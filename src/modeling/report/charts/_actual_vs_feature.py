...
def _determine_bin_size_parameter(
    ...
    return tp.cast(float, calculate_optimal_bin_width(series_for_bin_size_calculation))
...
def _define_perfect_alignment_line(
    target: pd.Series, is_residuals_mode: bool,
) -> _TAlignmentLine:
...
def _remove_distplot_y_ticks(fig: go.Figure, rows: tp.Iterable[int]) -> None:
...
def _link_x_axes_between_train_test_alignment_plots(  # noqa: WPS118
    fig: go.Figure,
) -> None:
...
def _update_layout(
    ...
    # there's a bug in plotly-js that renders hist with a tiny border for line_width=0
    # setting line_width to small number fixes that
    fig.update_traces(marker={"line_width": 1e-10}, col=2)
    _remove_distplot_y_ticks(fig=fig, rows=rows)
    _link_x_axes_between_train_test_alignment_plots(fig=fig)
    _link_plots(
        fig=fig,
        rows=rows,
        ref_axis="y1",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                ...
                "%{x" + formatting + "}<br>" + "%{y" + formatting + "}"  # noqa: WPS336
            ),
        ),
        row=row,
        col=col,
...
def _get_hist(
    ...
    is_vertical: bool,
    is_categorical: bool,
    bin_size: tp.Optional[float] = None,
) -> go.Histogram:
    if is_categorical:
        data_kwargs = (
...
def _get_density_line(
    ...
    kde = stats.gaussian_kde(series.dropna())(hist_domain)
    formatting = _get_float_precision_formatting(hover_precision)
    data_kwargs = (
        {
            "x": hist_domain,
