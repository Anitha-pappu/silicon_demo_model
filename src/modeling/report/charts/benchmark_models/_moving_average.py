...
from ._benchmark_model_base import BenchmarkModelBase  # noqa: WPS436
...
class MovingAverageBenchmarkModel(BenchmarkModelBase):
    def __init__(
        ...
        target: str,
        timestamp: str,
        time_window: str | None = "30D",
        target_rolling_average_name: str | None = None,
    ) -> None:
        """
    ...
    def get_feature_importance(
        ...
        # todo: update this once moving average is taken with the coef
    ...
    def _make_common_preparation_steps(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        - create a dataframe with only the ``self.timestamp`` and ``self._target``
            columns
        """
        timestamp_as_datetime = pd.to_datetime(data[self._timestamp])
        return data[[self.target, self._timestamp]].assign(**{
            self._timestamp: timestamp_as_datetime,
        })
    ...
    def _fit(self, data: pd.DataFrame, **kwargs: tp.Any) -> Self:
        ...
        df = (
            self
            ._make_common_preparation_steps(data)
            .dropna()
    ...
    def _predict(self, data: pd.DataFrame, **kwargs: tp.Any) -> pd.Series:
        ...
        """
        if self._fitted_target_rolling_average.empty:
            raise ValueError("The model is either not fitted or input is invalid")

        original_index = data.index
        df = self._make_common_preparation_steps(data).set_index(self._timestamp)
        y_pred = (
