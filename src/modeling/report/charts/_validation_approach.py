...
def _get_consecutive_periods(
    ...
    split_labels_ = pd.Series(split_labels).ffill().bfill().values
    if not len(split_labels_):
        return {}

    # last point of period included
    change_points = np.where(split_labels_[:-1] != split_labels_[1:])[0]

    consecutive_periods = defaultdict(list)
    period_start = 0
    for point in change_points:
        current_period = split_labels_[period_start]
        period_end = point + 1
        consecutive_periods[current_period].append((period_start, period_end))
        period_start = period_end
    current_period = split_labels_[period_start]
    consecutive_periods[current_period].append((period_start, len(split_labels_)))
...
def plot_validation_representation(
    ...
    split_labels = (
        data[timestamp_column].isin(train_data[timestamp_column])
    ...
    _add_lower_navigation_with_colored_instances(
        fig, train_data, test_data, timestamp_column,
    )
    _add_split_periods_highlights_to_reference_line(
        fig, data, split_labels, timestamp_column,
    )
    _update_layout(fig)
    return fig


def get_split_details(train_data: pd.DataFrame, test_data: pd.DataFrame) -> str:
    """
    Return a string specifying the size of train_data and test_data
    """
    train_size = train_data.shape[0]
    test_size = test_data.shape[0]
    return f"train size: {train_size}\ntest size: {test_size}"
...
def _plot_split_dataset(
    dataset: pd.DataFrame, timestamp_column: str, label: str,
) -> go.Scattergl:
    """
...
def _add_split_periods_highlights_to_reference_line(  # noqa: WPS118
    fig: go.Figure, data: pd.DataFrame, split_labels: pd.Series, timestamp_column: str,
) -> None:
...
def _update_layout(fig: go.Figure) -> None:
    fig.update_layout(
        ...
        height=HEIGHT,
        legend=dict(
            orientation="h", x=1, y=-0.1, xanchor="right", yanchor="top",
        ),
        hovermode="x unified",
        margin_pad=5,
...
def plot_consecutive_validation_periods(
    ...
    consecutive_periods_timestamps = _get_consecutive_periods_timestamps(
        data, split_column, timestamp_column, timestamp_format,
    )
    return {
        name: "\n".join(periods)
