...
class ModelBase(ABC):
    ...
    def get_feature_importance(
        ...
        """
        A method for getting feature importance from the model.
    ...
    @abstractmethod
    def _fit(self: Self, data: pd.DataFrame, **kwargs: tp.Any) -> Self:
        """
...
class ModelTunerBase(ABC):
    ...
    def __init__(
        self, model_factory: ModelFactoryBase, model_tuner_config: tp.Dict[str, tp.Any],
    ) -> None:
    ...
    def hyperparameters_config(self) -> tp.Optional[tp.Dict[str, tp.Any]]:
        if self._hyperparameters_config is None:
            return None
