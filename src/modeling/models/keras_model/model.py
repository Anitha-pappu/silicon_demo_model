...
class KerasModel(
    ...
    def _produce_shap_explanation(
        self, data: pd.DataFrame, algorithm: str = "deep", **kwargs: tp.Any,
    ) -> shap.Explanation:
        """
    ...
    def _fit(self, data: pd.DataFrame, **kwargs: tp.Any) -> Self:
        ...
        self._keras_model.fit(
            data[self._features_in], data[self._target], **modified_kwargs,
        )
        return self
    ...
    def _predict(
        ...
        **kwargs: tp.Any,
        ...
        """
        prediction: npt.NDArray["np.generic"] = self._keras_model.predict(
            data[self._features_in],
            **kwargs,
        ).reshape(-1)
        return prediction  # noqa: WPS331
    ...
    def _explain_using_deep_explainer(self, data: pd.DataFrame) -> shap.Explanation:
        explainer = shap.DeepExplainer(
            model=self._keras_model, data=data[self._features_in].values,
        )
