...
from modeling import api, types
...
class SklearnPipeline(  # noqa: WPS214
    ...
    def estimator(self) -> api.Estimator:
        ...
        """
        estimator: api.Estimator = self._pipeline[-1]
        return copy(estimator)

    def get_pipeline(  # noqa: WPS615
        self, select_features_step_name: str = "select_features",
    ) -> Pipeline:
        """
    ...
    def transform(self, data: pd.DataFrame, **kwargs: tp.Any) -> pd.DataFrame:
        ...
        if len(self._pipeline) > 1:
            transformed_data: pd.DataFrame = self._pipeline[:-1].transform(
                data,
                **kwargs,
            )
        else:
    ...
    def get_feature_importance(
        ...
        if hasattr(estimator, "feature_importances_"):  # noqa: WPS421
            ...
            feature_importance = permutation_importance(
                estimator, transformed_dataset, data[self._target],
            )["importances_mean"]
    ...
    def _produce_shap_explanation(
        self, data: pd.DataFrame, algorithm: str = "auto", **kwargs: tp.Any,
    ) -> shap.Explanation:
        """
        ...
        return self._explain_using_generic_explainer(
            data=data, algorithm=algorithm, **kwargs,
        )

    def _fit(self, data: pd.DataFrame, **kwargs: tp.Any) -> Self:
        """
        ...
        if len(self._pipeline) > 1:
            transformed_data = self._pipeline[:-1].fit_transform(
                data[self._features_in], data[self.target], **kwargs,
            )
        else:
    ...
    def _predict(self, data: pd.DataFrame, **kwargs: tp.Any) -> types.Vector:
        """
        ...
        vector: types.Vector = self._pipeline[-1].predict(transformed_data, **kwargs)
        return vector  # noqa: WPS331

    def _explain_using_generic_explainer(
        self, data: pd.DataFrame, algorithm: str, **kwargs: tp.Any,
    ) -> shap.Explanation:
    ...
    def _update_features_out_with_factual_columns(self, data: pd.DataFrame) -> None:
...
def check_transformed_columns(
    transformed_data: types.Matrix, expected_columns: tp.Optional[tp.Iterable[str]],
) -> None:
    """
    ...
    if transformed_columns is not None:
        if expected_columns != list(transformed_columns) or expected_columns is None:
            raise RuntimeError(
        ...
        logger.warning(
            ...
            "with `optimus_core.transformer` or new sklearn `set_output` API"
