...
def load_estimator(
    model_class: str, model_kwargs: tp.Optional[tp.Dict[str, tp.Any]] = None,
) -> api.Estimator:
    """
    ...
    estimator: api.Estimator = utils.load_obj(model_class)(**model_kwargs)
...
class SklearnModelFactory(ModelFactoryBase):
    ...
    def create(self) -> SklearnModel:
        ...
        return SklearnModel(
            estimator, target=self._target, features_in=self._features_in,
        )
