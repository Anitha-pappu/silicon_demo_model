...
def validate_estimator(estimator: api.Estimator) -> None:
...
class SklearnModel(ModelBase, ProducesShapFeatureImportance, EvaluatesMetrics):
    ...
    def __init__(
        self, estimator: api.Estimator, features_in: tp.Iterable[str], target: str,
    ):
        """
        Args:
    ...
    def get_feature_importance(
        ...
        if hasattr(estimator, "feature_importances_"):  # noqa: WPS421
            ...
            feature_importance = permutation_importance(
                estimator, data[self._features_in], data[self._target],
            )["importances_mean"]
