...
class WrapsKerasModel(tp.Protocol):
    ...
    `modeling.KerasModel` is aligned with this protocol definition.
    """
...
class KerasModelDataset(
    AbstractVersionedDataset[WrapsKerasModel, WrapsKerasModel],
):
    ...
    def __init__(
        ...
        self._pickle_dataset = PickleDataset(
            filepath=(Path(path) / self._FILENAME_FOR_WRAPPER_OBJECT).as_posix(),
            backend=backend,
        ...
        self._tensorflow_dataset = TensorFlowModelDataset(
            filepath=(Path(path) / self._DIR_NAME_FOR_TENSORFLOW_MODEL).as_posix(),
    ...
    @contextmanager
    def _tell_version_to_private_datasets(self) -> tp.Generator[None, None, None]:
        """
    ...
    def _load(self) -> WrapsKerasModel:
        ...
        with self._tell_version_to_private_datasets():
            ...
            class_model: WrapsKerasModel = self._pickle_dataset._load()
        if hasattr(class_model, "_keras_model"):  # noqa: WPS421
            class_model._keras_model = keras_model
            return class_model
        raise RuntimeError(
            "Error loading KerasModel, specified"
            " file is not a KerasModel instance.",
        )
    ...
    def _exists(self) -> bool:
        ...
        with self._tell_version_to_private_datasets():
            return tp.cast(
                bool,
                self._tensorflow_dataset._exists() and self._pickle_dataset._exists(),
            )
